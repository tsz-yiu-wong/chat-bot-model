...
def startup_event():
    ...
    if tokenizer is None or model is None or model_name is None:
        print("❌ 模型加载失败！")
        exit(1)
    print("✅ 模型加载成功！")
...
async def create_chat_completion(payload: ChatCompletionRequest, request: Request, authorization: Optional[str] = Header(None)):
    """处理聊天请求，返回AI生成的完整回复。"""
    try:
